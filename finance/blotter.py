...
class Blotter(object):
    def __init__(self, data_frequency, asset_finder, volume_share_slippage=VolumeShareSlippage(),
                 commission=PerShare(), cancel_policy=None):
        ...
        self.volume_share_slippage = volume_share_slippage 
    ...
    def __repr__(self):
        return """
{class_name}(
    slippage={volume_share_slippage},
...
""".strip().format(class_name=self.__class__.__name__,
                   volume_share_slippage=self.volume_share_slippage,
    ...
    def order(self, sid, amount, order_style, order_id=None):
        ...
        #FIXME: Clarify that if amount is negative, it means we are selling the orders
        ...
        order = Order(
            ...
            sid=sid,
            amount=amount,
            stop=order_style.get_stop_price(is_buy),
            limit=order_style.get_limit_price(is_buy),
            id=order_id
        )
    ...
    def get_transactions(self, bar_data):
        ...
        if self.open_orders:
            ...
            for sid, asset_orders in iteritems(self.open_orders):
                ...
                for order, txn in \
                        self.volume_share_slippage(bar_data, asset, asset_orders):
